...
def generer_carte(
    width=355,
    height=175,
    nb_obj_min=6, nb_obj_max=6,
    obj_w_min=25, obj_w_max=75,
    obj_h_min=25, obj_h_max=75,
    min_gap=16,
    max_attempts=300
):
...
def sauvegarder_cartes(n=5, dossier="./maps", seed=42):
    if seed is not None:
        random.seed(seed)
        np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Générateur de cartes aléatoires.")
    
    # Optionnel : nombre de carte à générer
    parser.add_argument(
        "-n", "--number", 
        type=int, 
        default=10, 
        help="Nombre de cartes à générer (défaut: 10)"
    )
    
    # Optionnel : permettre de changer le dossier de sortie aussi
    parser.add_argument(
        "-o", "--output", 
        type=str, 
        default="./maps", 
        help="Dossier de sortie (défaut: ./NAO-S5/Maps/maps)"
    )

    # Optionnel : seed à utiliser
    parser.add_argument(
        "-s", "--seed", 
        type=str, 
        default=42, 
        help="Seed à utiliser (défaut: 42)"
    )

    args = parser.parse_args()

    # Appel de la fonction avec les arguments récupérés
    sauvegarder_cartes(n=args.number, dossier=args.output, seed=args.seed)